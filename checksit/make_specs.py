...
def make_amof_specs(version_number):
    ...
    for attr in data.keys():
        ...
        if compliance.lower() in ["exact match", "exact match of text to the left"]:
            ...
            if attr == "source":
                rule = (
                    f"__vocabs__:AMF_CVs/{version_number}/AMF_ncas_instrument:"
                    "ncas_instrument:__all__:description"
                )
            elif attr == "platform":
                rule = (
                    f"__vocabs__:AMF_CVs/{version_number}/AMF_platform:platform:__all__"
                )
            else:
                # a few extra catches
                if attr == "institution":
            ...
            with open(f"{cvs_dir}/AMF_product_{product}_global-attributes.json") as f:
                ...
                for attr in data.keys():
                    ...
                    if compliance.lower() in [
                        ...
                        if attr == "source":
                            rule = (
                                "__vocabs__:AMF_CVs/AMF_ncas_instrument:"
                                "ncas_instrument:__all__:description"
                            )
                        elif attr == "platform":
